...
export default class Inventory extends Vue {
  ...
  getProducts() {
    this.$inventory
      .getAllProducts(this.page, this.numOfItems)
      .then((data: NotWellDefinedObject[]) => {
        this.products = data
      })
      .catch((err) => console.error(err))
  ...
  async mounted() {
    const productContainer = (this.$refs['productContainer'] as Vue)
      .$el as HTMLElement
    const productsCount = await this.$inventory.getProductsCount()
    if (productContainer != undefined) {
      this.numOfItems = Math.floor((productContainer.scrollHeight - 8) / 64)
      this.numOfPages = Math.ceil(productsCount / this.numOfItems)
    }
  }
